        ...
        ['$stateProvider', '$urlRouterProvider',
            function ($stateProvider, $urlRouterProvider) {
                $stateProvider
                    .state('index.gestion', {
                        ...
                        views: {
                            '': {
                                ...
                                controller: ['$scope', '$state', '$base64', 'toastr', 'authService', 'consultaService', 'gestionService',
                                    function ($scope, $state, $base64, toastr, authService, consultaService, gestionService) {
                                        ...
                                        $scope.buscarNoCaso = function (KeyEvent) {
                                            if (KeyEvent.which === 13) {
                                                if ($scope.busqueda == null || $scope.busqueda == "") {
                                                    ...
                                                    gestionService.consultaEnDependencia($scope.busqueda).then(function (response) {
                                                        if (response.status == 'OK') {
                                                            ...
                                                            switch (response.data[0].estatus) {
                                                                ...
                                                                case 'C': $scope.estadoCaso = "CANCELADO";
                                                                    break;
                                                                case 'E': $scope.estadoCaso = "E";
                                                                    break;
                                                            }
                                                            gestionService.consultaCaso($scope.busqueda).then(function (res) {
                                                                if (res.data.length > 0) {
                                                                    ...
                                                                    swal({
                                                                        title: "No permitido",
                                                                        text: "El caso ya cuenta con PIN.",
                                                                        type: "warning",
                                                                        confirmButtonClass: "btn-success",
                                                                    })
                                                                }
                                        ...
                                        $scope.random = function () {
                                            if ($scope.busqueda == null || $scope.busqueda == "" || $scope.estadoCaso == null) {
                                                toastr.error("No se puede activar esta función. Datos requeridos");
                                            }
                                            if (($scope.busqueda != null || $scope.busqueda != "" && $scope.estadoCaso == null) && $scope.identificacion == null) {

                                                swal({
                                                    title: "No permitido",
                                                    text: "No se puede activar esta función. Documento de Identificación Personal requerido.",
                                                    type: "error",
                                                    closeOnConfirm: true,
                                            ...
                                            if (($scope.busqueda != null) && $scope.estadoCaso != null && $scope.estadoCaso != 'VIGENTE') {
                                                swal({
                                                    title: "No permitido",
                                                    text: "El caso no se encuentra vigente",
                                                    type: "warning",
                                            ...
                                            if (($scope.busqueda != null) && $scope.estadoCaso != null && $scope.estadoCaso == 'VIGENTE' && $scope.identificacion != null) {
                                                //gestionService.consultaCaso($scope.busqueda).then(function(response){
                                                //if(response.data.length == 0){
                                                //almacenarNuevoCaso&pin
                                                if (($scope.identificacion.length == 13)) {

                                                    $scope.checked = 1;
                                                    var r = Math.floor(Math.random() * (9999 - 1000) + 1000)
                                                    $scope.resultado = r; $scope.resultado = $base64.encode($scope.resultado);
                                                    $scope.fl = $scope.fl + 1;
                                                } else {
                                                    $scope.fl = 0;
                                                    toastr.warning("Se debe ingresar DPI valido. ");
                                                }
                                                /*if ($scope.identificacion == null || $scope.identificacion == ""){
                                                   $scope.fl = 0;
                                                   toastr.warning ("No se puede proceder sin ingresar DPI valido. ");
                                                } */
                                                /*}
                                                else{
                                                    $scope.fl = null;                               
                                                    swal({
                                                        title: "No permitido",
                                                        text: "No se puede duplicar PIN para Caso",
                                                        type: "error",
                                                        timer : 5000
                                                      })     
                                                }*/
                                        ...
                                        $scope.reg = function () {
                                            ...
                                            if ($scope.identificacion == null || $scope.identificacion == "" || $scope.resultado == null || $scope.resultado == "" || $scope.estadoCaso == null || $scope.estadoCaso == "") {
                                                swal({
                                                    title: "No permitido",
                                                    text: "Campos vacíos.",
                                                    closeOnConfirm: true,
                                            ...
                                            if ($scope.identificacion == null && $scope.estadoCaso != null) {
                                                $scope.caracterFaltante = $scope.identificacion.length;

                                                //alert($scope.caracterFaltante);
                                                swal({
                                                    title: "No permitido",
                                                    text: "Se debe ingresar un Número de DPI valido.",
                                                    type: "warning",
                                                    //showCancelButton: true,
                                                    //confirmButtonClass: "btn-success",
                                                    //confirmButtonText: "Confirmar",
                                            ...
                                            if ($scope.resultado != null && ($scope.identificacion != null && $scope.identificacion.length < 13)) {
                                                $scope.caracterFaltante = 13 - $scope.identificacion.length;

                                                swal({
                                                    title: "No permitido",
                                                    text: "Se debe ingresar un Número de DPI Valido. Se requiere " + $scope.caracterFaltante + " dato(s) del DPI ingresado.",
                                                    type: "warning",
                                                    closeOnConfirm: true,
                                                })
                                            }
                                            if ($scope.fl > 0 && $scope.identificacion != "" && $scope.resultado != null && ($scope.identificacion.length == 13)) {
                                                $scope.ban = 1;
                                                if ($scope.ban == 1 && $scope.resultado != null && ($scope.identificacion.length == 13)) {
                                                    $scope.valprint = 1;
                                                    var data = {
                                                        fkCaso: $scope.busqueda,
                                                        pin: $scope.resultado,
                                                        usuarioCrea: $scope.loginData.usuario,
                                                        nombreUsuarioCrea: $scope.loginData.usuarioNombre.toUpperCase(),
                                                        gafete: $scope.loginData.codigoEmpleado,
                                                        dependenciaCrea: $scope.gblDependencia,
                                                        dpiCrea: $scope.identificacion,
                                                        rhDependencia: $scope.gblDependenciaId
                                                    }
                                                    gestionService.add(data).then(function (response) {
                                                        if (response.status == 'OK') {
                                                            //toastr.success( response.message );
                                                            swal({
                                                                title: "Registro exitoso",
                                                                text: "El PIN ha sido creado. Puede generar la constancia ahora.",
                                                                closeOnConfirm: true,
                                        ...
                                        $scope.print = function () {
                                            var bodyTable = [];
                                            if ($scope.valprint > 0) {
                                                $scope.val = $base64.decode($scope.resultado);
                                                bodyTable.push([
                                                    { text: $scope.busqueda, alignment: 'center', fontSize: '10' },
                                                    { text: $scope.nombreCaso, fontSize: '9' },
                                                    { text: $scope.val, alignment: 'center', fontSize: '9' }
                                                ]);
                                                ...
                                                var docDefinition = {
                                                        ...
                                                        return {
                                                            ...
                                                            {margin: [75, 0, 0, 0],
                                                                columns: [
                                                                    ...
                                                                    { text: 'Usuario Imprime: ' + $scope.loginData.usuario, fontSize: '8' },
                                                                    { text: moment(new Date()).format("DD-MM-YYYY hh:mm:ss a"), alignment: 'left', fontSize: '8', margin: [0, 0, 20, 0] }
                                                                ]
                                                            }]
                                                    ...
                                                    content: [
                                                        ...
                                                        { text: '\n', alignment: 'center', fontSize: '10' }, { text: 'Usuario Imprime: ' + $scope.loginData.usuario, fontSize: '8', alignment: 'rigth' },
                                                        { text: moment(new Date()).format("DD-MM-YYYY hh:mm:ss a"), alignment: 'left', fontSize: '8', margin: [0, 0, 20, 0] },
...

                                                        {
                                                            table: {
                                                                ...
                                                                widths: [70, 190, 190],
                                                                body: [
                                                                    [{ text: 'CASO', alignment: 'center', fontSize: '11' }, { text: 'NOMBRE BENEFICIARIO', alignment: 'center', fontSize: '11' }, { text: 'PIN', alignment: 'center', fontSize: '11' }],
                                                                ]
                                                            }
                                                        },
                                                        {
                                                        ...
                                                        { text: '\n', alignment: 'justify', fontSize: '10' },
                                                        { text: 'Pase atentamente a: ' + $scope.nombreCaso, alignment: 'justify', fontSize: '10' },
                                                        { text: '\n', alignment: 'justify', fontSize: '10' },
                                                        { text: 'Se recomienda cambiar el PIN proporcionado, cuando utilice por primera vez el Sistema de Consulta de Saldo de Pension Alimenticia, ingresando a la página http://pensionesalimenticias.oj.gob.gt', alignment: 'justify', fontSize: '09' },
                                                        { text: '\n', alignment: 'justify', fontSize: '10' },
                                                        { text: 'También puede realizar la consulta y utilizar otros servicios a través de la aplicación del Organismo Judicial, disponible en PlayStore y AppStore.', alignment: 'justify', fontSize: '09' }
                                                    ]
                                                ...
                                                toastr.error("No es posible ejecutar la funcion");
                                            }
                    ...
                    .state('index.gestion-notificacion', {
                        ...
                        views: {
                            '': {
                                templateUrl: 'app/gestion/adhesion.tpl.html',
                                resolve: {
                                    moneda: ['casoService',
                                ...
                                controller: ['$scope', '$state', '$sce', 'toastr', 'ngDialog', 'moneda', 'dependencia', 'documento', 'persona','empresa','casoService', 'authService','consultaService',
                                    function ($scope, $state, $sce, toastr, ngDialog, moneda, dependencia, documento, persona, empresa, casoService, authService,consultaService) {
                                        ...
                                        $scope.buscarNoCaso = function (KeyEvent) {
                                            console.log("AFSDFDSAFDSAFDSA")
                                            if (KeyEvent.which === 13) {
                                                var a = $scope.busqueda;
                                                if ($scope.consulta.busqueda == null || $scope.consulta.busqueda == "" || $scope.consulta.busqueda == undefined) {
                                                    toastr.error("Campo vacio. Ingrese numero de caso");
                                                } else {

                                                    consultaService.consultaEstado($scope.consulta.busqueda).then(function (response) {
                                                        if (response.data == null || response.data[0] == undefined) {
                                                            toastr.warning("El número de caso ingresado no es correto");
                                                        }
                                                        if (response.status == 'OK') {
                                                            $scope.nombreCaso = response.data[0].beneficiario;
                                                            $scope.nombreDepositante = response.data[0].depositante; //va sin costo
                                                            //$scope.estadoCaso = response.data[0].estatus;
                                                            switch (response.data[0].estatus) {
                                                                case "V": $scope.estadoCaso = "VIGENTE";
                                                                    break;
                                                            ...
                                                            if ((response.data[1] != null) || (response.data[1] != undefined)) {
                                                                $scope.casoImpreso.data = response.data;
                                                            }
                                                        } else {
                                                            toastr.error(response.message);
                                                        }
                                                    }, function (error) {
                                                        toastr.error("Fallo de comunicación del servicio a base de datos. Cx001C1");
                                                    }) //finServicio                                                                                                   
                                                }
                                            }
                                        }
